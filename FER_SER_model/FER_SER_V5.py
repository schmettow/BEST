##FER_SER_v5 Emotion Detection FOR WINDOWS OPERATING SYSTEMS !!!
...
while True:
 ...
 # Convert frame to grayscale
    ...
    roi_x = max(0, center_x - roi_size)
    roi_y = max(0, center_y - roi_size)
    roi_width = min(width - roi_x, roi_size * 2)
    roi_height = min(height - roi_y, roi_size * 2)

    # Extract Region of Interest
    roi = gray[roi_y:roi_y + roi_height, roi_x:roi_x + roi_width]

    # # OPTIONAL: Show ROI window for debugging
    # cv2.imshow("ROI", roi)

    # Safeguard: skip frame if ROI is empty
    if roi.size == 0:
        continue

# Detect faces in the ROI with relaxed parameters
...
# Adjust detected face coordinates back to full-frame coordinates
